(() => {
  ...
  async function bootCritical(){
    ...
    if(!router){console.warn("v73.7 map router not ready");return false;}
  ...
  window.addEventListener("load",()=>{apply();setTimeout(()=>{window.OnsenMapDomainV73?.refresh?.();window.OnsenScenicRendererV734?.refresh?.();window.OnsenMapDiscoveryV737?.refresh?.();},100);});
