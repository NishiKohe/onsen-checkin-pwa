(() => {
  ...
  async function ensureScenicRuntime(){
    ...
    if(!rt)console.warn("v73.10 scenic runtime 433 entries not ready");
    return rt;
  }
  async function ensureScenicAdapter(){
    ...
    if(!scenic)console.warn("v73.10 scenic interaction adapter not ready");
    return scenic;
  }
  async function ensureScenicRenderer(){
    ...
    if(!renderer)console.warn("v73.10 scenic renderer did not reach 433 features",window.OnsenScenicRendererV734?.diagnostics?.());
    return renderer;
  }
  async function bootCritical(){
    ...
    if(!router){console.warn("v73.10 map router not ready");return false;}
    const castle=await ensureCastleAdapter();if(!castle)console.warn("v73.10 castle adapter not ready");
    ...
    await loadScriptOnce("./map-detail-compact-v737.js?v=73.7","mapDetailCompactV737Script");
  ...
  async function registerPreferredWorker(){if(!("serviceWorker" in navigator))return null;try{const registration=await navigator.serviceWorker.register(preferredWorker);await registration?.update?.();return registration;}catch(err){console.warn("v73.10 service worker update check skipped",err);return null;}}
  function patchServiceWorkerRegister(){if(!("serviceWorker" in navigator))return;const sw=navigator.serviceWorker;if(sw.__onsenV7310RegisterPatched)return;try{const originalRegister=sw.register.bind(sw);sw.register=(scriptURL,options)=>/(?:^|\/)sw\.js(?:\?|$)/.test(String(scriptURL||""))?originalRegister(preferredWorker,options):originalRegister(scriptURL,options);Object.defineProperty(sw,"__onsenV7310RegisterPatched",{value:true});}catch(error){console.warn("v73.10 service worker register patch skipped",error);}}
  ...
  const start=()=>{apply();bootCritical().catch((error)=>console.warn("v73.10 critical bootstrap failed",error));};
