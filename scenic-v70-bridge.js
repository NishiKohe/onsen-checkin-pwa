(() => {
  ...
  async function boot(){
    ...
    if(!await waitFor(()=>window.OnsenMapDomainV72?.build===ROUTER_BUILD,12000)){console.warn("v72 scenic bridge: map router not ready");return false;}
    if(!await waitFor(()=>document.querySelector("#collectionView .collection-shell"),8000)){console.warn("v72 scenic bridge: collection shell not ready");return false;}
    if(window.OnsenScenicRuntime?.build!==BUILD){if(!await loadScriptOnce("./scenic-runtime-v70.js?v=72","scenicRuntimeV71"))return false;}
    if(!await waitFor(()=>window.OnsenScenicRuntime?.build===BUILD,12000)){console.warn("v72 scenic bridge: runtime not ready");return false;}
    if(!await waitFor(()=>window.OnsenCastleCollectionUI&&document.querySelector("#collectionView .collection-domain-switch"),10000)){console.warn("v72 scenic bridge: canonical collection domain not ready");return false;}
    ...
    if(!await waitFor(()=>window.OnsenScenicCollectionUI?.build===BUILD,10000)){console.warn("v72 scenic bridge: collection UI not ready");return false;}
    if(window.OnsenScenicMapV71?.build!==BUILD){await loadScriptOnce("./scenic-map-v71.js?v=72","scenicMapV71Script");}
    if(!await waitFor(()=>window.OnsenScenicMapV71?.build===BUILD,10000)){console.warn("v72 scenic bridge: map UI not ready");return false;}
    ...
    window.dispatchEvent(new CustomEvent("onsen-scenic-v71-ready",{detail:{build:BUILD,router:ROUTER_BUILD,progress:window.OnsenScenicRuntime?.progress?.()||null}}));return true;
  }
  function install(){if(installed)return;installed=true;const api={build:BUILD,routerBuild:ROUTER_BUILD,boot};window.OnsenScenicV71Bridge=api;window.OnsenScenicV70Bridge=api;boot().catch((error)=>console.warn("v72 scenic bridge boot failed",error));}
