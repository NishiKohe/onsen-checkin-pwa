(() => {
  ...
  async function boot(){
    addStyle("./scenic-collection-ui-v70.css?v=70.13","scenicCollectionStyleV70");
    if(!await waitFor(()=>document.querySelector("#collectionView .collection-shell"),8000)){console.warn("v70.13 scenic bridge: collection shell not ready");return false;}
    if(!window.OnsenScenicRuntime||window.OnsenScenicRuntime.build!==BUILD){
      if(!await loadScriptOnce("./scenic-runtime-v70.js?v=70.13","scenicRuntimeV7013"))return false;
    }
    if(!await waitFor(()=>window.OnsenScenicRuntime?.build===BUILD,12000)){console.warn("v70.13 scenic bridge: runtime not ready");return false;}
    if(!await waitFor(()=>window.OnsenCastleCollectionUI&&document.querySelector("#collectionView .collection-domain-switch"),10000)){console.warn("v70.13 scenic bridge: canonical collection domain not ready");return false;}
    if(!window.OnsenScenicCollectionUI){if(!await loadScriptOnce("./scenic-collection-ui-v70.js?v=70.13","scenicCollectionUiV70"))return false;}
    if(!await waitFor(()=>window.OnsenScenicCollectionUI,10000)){console.warn("v70.13 scenic bridge: collection UI not ready");return false;}
    ...
    window.dispatchEvent(new CustomEvent("onsen-scenic-v7013-ready",{detail:{build:BUILD,progress:window.OnsenScenicRuntime?.progress?.()||null}}));return true;
  }
  function install(){if(installed)return;installed=true;boot().catch((error)=>console.warn("v70.13 scenic bridge boot failed",error));window.OnsenScenicV70Bridge={build:BUILD,boot};}
