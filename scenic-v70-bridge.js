(() => {
  ...
  async function boot(){
    ...
    addStyle("./scenic-map-v71.css?v=71.2","scenicMapStyleV71");
    addStyle("./scenic-map-hotfix-v712.css?v=71.2","scenicMapHotfixStyleV712");
    ...
    if(window.OnsenScenicRuntime?.build!==BUILD){if(!await loadScriptOnce("./scenic-runtime-v70.js?v=71.2","scenicRuntimeV71"))return false;}
    ...
    if(window.OnsenScenicCollectionUI?.build!==BUILD){if(!await loadScriptOnce("./scenic-collection-ui-v70.js?v=71.2","scenicCollectionUiV71"))return false;}
    ...
    if(window.OnsenScenicMapV71?.build!==BUILD){await loadScriptOnce("./scenic-map-v71.js?v=71.2","scenicMapV71Script");}
    if(!await waitFor(()=>window.OnsenScenicMapV71?.build===BUILD,10000)){console.warn("v71 scenic bridge: map UI not ready");return false;}
    if(!window.OnsenScenicMapHotfixV712){await loadScriptOnce("./scenic-map-hotfix-v712.js?v=71.2","scenicMapHotfixV712Script");}
    ...
    window.dispatchEvent(new CustomEvent("onsen-scenic-v71-ready",{detail:{build:BUILD,hotfix:"v71.2",progress:window.OnsenScenicRuntime?.progress?.()||null}}));return true;
  }
