(() => {
  ...
  const LEGACY_MIGRATION_KEY = "gameStateProfileMigrationV59";
  const MAX_ENERGY = 12;
  ...
  function defaultState() {
    return {
      ...
      fishing: defaultFishingState(),
      unlocks: { fishing: true, rogue: false },
      notifications: { newFish: 0 },
      debug: { lastAction: null, lastActionAt: null },
      updatedAt: Date.now()
    };
  }
  ...
  function userStorage() {
    return window.OnsenUserStorage || null;
  }

  function migrateLegacyGameStateOnce() {
    const storage = userStorage();
    if (!storage?.readUserItem || !storage?.writeUserItem) return;
    const profileId = storage.getCurrentProfileId?.() || "current";
    const marker = `${LEGACY_MIGRATION_KEY}:${profileId}`;
  ...
  function loadState() {
    return normalizeState(readStateRaw());
  }
  ...
  function debugRefillEnergy() {
    const state = loadState();
    state.energy.value = state.energy.max || MAX_ENERGY;
    state.debug.lastAction = "refill_energy";
    state.debug.lastActionAt = Date.now();
    return saveState(state, "debug_refill_energy");
  }

  function debugResetFishing() {
    const state = loadState();
    state.fishing = defaultFishingState();
    state.notifications.newFish = 0;
    state.debug.lastAction = "reset_fishing";
    state.debug.lastActionAt = Date.now();
    return saveState(state, "debug_reset_fishing");
  }
